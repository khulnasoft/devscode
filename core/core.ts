...
import { DevscodeServerClient } from "./devscodeServer/stubs/client";
...
import { editConfigJson, setupInitialDotDevscodeDirectory } from "./util/paths";
...
export class Core {
  ...
  constructor(
    ...
    // Ensure .devscode directory is created
    setupInitialDotDevscodeDirectory();
    ...
    // Codebase Indexer and DevscodeServerClient depend on IdeSettings
    let codebaseIndexerResolve: (_: any) => void | undefined;
    this.codebaseIndexerPromise = new Promise(
      async (resolve) => (codebaseIndexerResolve = resolve),
    );

    let devscodeServerClientResolve: (_: any) => void | undefined;
    this.devscodeServerClientPromise = new Promise(
      (resolve) => (devscodeServerClientResolve = resolve),
    );

    void ideSettingsPromise.then((ideSettings) => {
      const devscodeServerClient = new DevscodeServerClient(
        ideSettings.remoteConfigServerUrl,
        ideSettings.userToken,
      );
      devscodeServerClientResolve(devscodeServerClient);
    ...
    on("didChangeActiveTextEditor", async ({ data: { filepath } }) => {
      ...
      try {
        ...
        if (e instanceof RangeError) {
          // do nothing, this can happen when editing a file outside the workspace such as `../extensions/.devscode-debug/config.json`
        } else {
