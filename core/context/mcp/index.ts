...
import { DevscodeConfig, MCPOptions, SlashCommand, Tool } from "../..";
...
class MCPConnectionSingleton {
  ...
  async modifyConfig(config: DevscodeConfig): Promise<DevscodeConfig> {
    try {
    ...
    const devscodeTools: Tool[] = tools.map((tool) => ({
    ...
    config.tools = [...config.tools, ...devscodeTools];
