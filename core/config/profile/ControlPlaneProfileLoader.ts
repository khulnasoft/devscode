import { ConfigJson } from "@khulnasoft/config-types";
...
export default class ControlPlaneProfileLoader implements IProfileLoader {
  ...
  async doLoadConfig(): Promise<ConfigResult<DevscodeConfig>> {
    const settings =
      this.workspaceSettings ??
      ((await this.controlPlaneClient.getSettingsForWorkspace(
        this.profileId,
      )) as any);
    const serializedConfig: SerializedDevscodeConfig = settings;
