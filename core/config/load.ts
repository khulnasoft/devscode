...
import DevscodeProxyContextProvider from "../context/providers/DevscodeProxyContextProvider";
...
function resolveSerializedConfig(filepath: string): SerializedDevscodeConfig {
  let content = fs.readFileSync(filepath, "utf8");
  const config = JSONC.parse(content) as unknown as SerializedDevscodeConfig;
  ...
  return JSONC.parse(content) as unknown as SerializedDevscodeConfig;
}
...
function loadSerializedConfig(
  workspaceConfigs: DevscodeRcJson[],
  ideSettings: IdeSettings,
  ideType: IdeType,
  overrideConfigJson: SerializedDevscodeConfig | undefined,
): ConfigResult<SerializedDevscodeConfig> {
  const configPath = getConfigJsonPath(ideType);
  let config: SerializedDevscodeConfig = overrideConfigJson!;
  if (!config) {
    try {
...
async function serializedToIntermediateConfig(
  initial: SerializedDevscodeConfig,
  ide: IDE,
  ...
  if (loadPromptFiles) {
    ...
    // Also read from ~/.devscode/.prompts
...
async function intermediateToFinalConfig(
  ...
  for (const provider of config.contextProviders || []) {
    if (isContextProviderWithParams(provider)) {
      ...
      // Handle devscode-proxy
      if (instance.description.title === "devscode-proxy") {
        (instance as DevscodeProxyContextProvider).workOsAccessToken =
          workOsAccessToken;
      }
  ...
  let devscodeConfig: DevscodeConfig = {
    ...config,
    contextProviders,
    models,
  ...
  if (config.experimental?.modelContextProtocolServer) {
    ...
    devscodeConfig = await Promise.race<DevscodeConfig>([
      mcpConnection.modifyConfig(devscodeConfig),
      ...
      return devscodeConfig; // Return original config if timeout occurs
    });
  }

  return devscodeConfig;
}

function finalToBrowserConfig(
  final: DevscodeConfig,
): BrowserSerializedDevscodeConfig {
  return {
...
async function tryBuildConfigTs() {
  try {
    ...
    console.log(
      `Build error. Please check your ~/.devscode/config.ts file: ${e}`,
    );
  }
}
...
export {
  ...
  type BrowserSerializedDevscodeConfig,
};
