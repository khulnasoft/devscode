...
type ConfigUpdateFunction = (payload: ConfigResult<DevscodeConfig>) => void;
...
export class ConfigHandler {
  ...
  async openConfigProfile(profileId?: string) {
    ...
    if (openProfileId === "local") {
      ...
      await this.ide.openUrl(
        "https://app.devscode.dev/",
        // `https://app.devscode.dev/workspaces/${openProfileId}/chat`,
      );
    }
  }
  ...
  // Automatically refresh config when Devscode-related IDE (e.g. VS Code) settings are changed
  ...
  private notifyConfigListeners(result: ConfigResult<DevscodeConfig>) {
  ...
  getSerializedConfig(): Promise<BrowserSerializedDevscodeConfig> {
  ...
  async loadConfig(): Promise<DevscodeConfig> {
