...
import { IDevscodeServerClient } from "../devscodeServer/interface.js";
import {
  BranchAndDir,
  Chunk,
  EmbeddingsProvider,
...
export class LanceDbIndex implements CodebaseIndex {
  ...
  constructor(
    ...
    private readonly devscodeServerClient?: IDevscodeServerClient,
  ) {}
  ...
  async *update(
    ...
    // Check remote cache
    if (this.devscodeServerClient?.connected) {
      try {
        const keys = results.compute.map(({ cacheKey }) => cacheKey);
        const resp = await this.devscodeServerClient.getFromIndexCache(
          keys,
          "embeddings",
          repoName,
        );
        for (const [cacheKey, chunks] of Object.entries(resp.files)) {
          ...
          if (!path) {
            console.warn(
              "Devscode server sent a cacheKey that wasn't requested",
              cacheKey,
            );
            devscode;
          }
