...
describe("walkDir", () => {
  ...
  test("should listen to both .gitignore and .devscodeignore", async () => {
    const files = [
      [".gitignore", "*.py"],
      [".devscodeignore", "*.ts"],
      "a.txt",
  ...
  test("should walk devscode repo without getting any files of the default ignore types", async () => {
    const results = await walkDir(path.join(__dirname, ".."), ide, {
      ignoreFiles: [".gitignore", ".devscodeignore"],
    });
  ...
  test.skip("should walk devscode/extensions/vscode without getting any files in the .devscodeignore", async () => {
