...
**Here is a step-by-step guide on how to set up Codestral with Devscode using the Mistral AI API:**

1. Install the Devscode VS Code or JetBrains extension following the instructions [here](../../getting-started/install.md)

2. Click on the gear icon in the bottom right corner of the Devscode window to open `~/.devscode/config.json` (MacOS) / `%userprofile%\.devscode\config.json` (Windows)
