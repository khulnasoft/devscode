import { SerializedDevscodeConfig } from "core";
...
const DEVSCODE_GLOBAL_DIR = path.join(__dirname, "..", ".devscode");
if (fs.existsSync(DEVSCODE_GLOBAL_DIR)) {
  fs.rmSync(DEVSCODE_GLOBAL_DIR, { recursive: true, force: true });
}
fs.mkdirSync(DEVSCODE_GLOBAL_DIR);
...
describe("Test Suite", () => {
  ...
  beforeAll(async () => {
    ...
    const binaryPath = path.join(binaryDir, `devscode-binary${exe}`);
    const expectedItems = [
      `devscode-binary${exe}`,
      `esbuild${exe}`,
      "index.node",
    ...
    if (USE_TCP) {
      ...
      try {
        subprocess = spawn(binaryPath, {
          env: { ...process.env, DEVSCODE_GLOBAL_DIR },
        });
  ...
  it("should create .devscode directory at the specified location with expected files", async () => {
    expect(fs.existsSync(DEVSCODE_GLOBAL_DIR)).toBe(true);
    ...
    const missingFiles = expectedFiles.filter((file) => {
      const filePath = path.join(DEVSCODE_GLOBAL_DIR, file);
  ...
  it("should add and delete a model from config.json", async () => {
    const model: SerializedDevscodeConfig["models"][number] = {
      title: "Test Model",
      provider: "openai",
      model: "gpt-3.5-turbo",
    };
