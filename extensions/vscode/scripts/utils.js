...
const devscodeDir = path.join(__dirname, "..", "..", "..");

function copyConfigSchema() {
  // Modify and copy for .devscoderc.json
  const schema = JSON.parse(fs.readFileSync("config_schema.json", "utf8"));
  schema.definitions.SerializedDevscodeConfig.properties.mergeBehavior = {
    type: "string",
    enum: ["merge", "overwrite"],
    default: "merge",
    title: "Merge behavior",
    markdownDescription: "If set to 'merge', .devscoderc.json will be applied on top of config.json (arrays and objects are merged). If set to 'overwrite', then every top-level property of .devscoderc.json will overwrite that property from config.json.",
    "x-intellij-html-description": "<p>If set to <code>merge</code>, <code>.devscoderc.json</code> will be applied on top of <code>config.json</code> (arrays and objects are merged). If set to <code>overwrite</code>, then every top-level property of <code>.devscoderc.json</code> will overwrite that property from <code>config.json</code>.</p>"
  };
  fs.writeFileSync("devscode_rc_schema.json", JSON.stringify(schema, null, 2));
...
function installNodeModules() {
  ...
  if (!process.cwd().endsWith("vscode")) {
    process.chdir(path.join(devscodeDir, "extensions", "vscode"));
  }
  ...
  process.chdir(path.join(devscodeDir, "gui"));
...
//     execCmdSync(
//       `curl -o node_modules/@esbuild/esbuild.zip https://devscode-server-binaries.s3.us-west-1.amazonaws.com/${target}/esbuild.zip`,
//     );
...
async function installNodeModuleInTempDirAndCopyToCurrent(packageName, toCopy) {
  ...
  const tempDir = `/tmp/devscode-node_modules-${adjustedName}`;
