...
import { DevscodeCompletionProvider } from "../autocomplete/completionProvider";
import {
...
export class VsCodeExtension {
  ...
  private sidebar: DevscodeGUIWebviewViewProvider;
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.sidebar = new DevscodeGUIWebviewViewProvider(
    ...
    const outputChannel = vscode.window.createOutputChannel(
      "Devscode - LLM Prompt/Completion",
    );
    ...
    context.subscriptions.push(
      vscode.languages.registerInlineCompletionItemProvider(
        [{ pattern: "**" }],
        new DevscodeCompletionProvider(
          this.configHandler,
          this.ide,
    ...
    registerAllCommands(
      ...
      this.diffManager,
      this.verticalDiffManager,
      this.core.devscodeServerClientPromise,
      this.battery,
      quickEdit,
      this.core,
      this.editDecorationManager,
    ...
    vscode.workspace.onDidSaveTextDocument(async (event) => {
      ...
      if (
        filepath.endsWith(".devscoderc.json") ||
        filepath.endsWith(".prompt") ||
        filepath.endsWith(SYSTEM_PROMPT_DOT_FILE)
      ) {
        this.configHandler.reloadConfig();
      } else if (
        filepath.endsWith(".devscodeignore") ||
    ...
    vscode.authentication.onDidChangeSessions(async (e) => {
      if (e.provider.id === controlPlaneEnv.AUTH_TYPE) {
        ...
        this.webviewProtocolPromise.then(async (webviewProtocol) => {
          ...
          // To make sure devscode-proxy models and anything else requiring it get updated access token
          this.configHandler.reloadConfig();
        });
    ...
    context.subscriptions.push(
      vscode.workspace.registerTextDocumentContentProvider(
        VsCodeExtension.devscodeVirtualDocumentScheme,
  ...
  static devscodeVirtualDocumentScheme = EXTENSION_NAME;
