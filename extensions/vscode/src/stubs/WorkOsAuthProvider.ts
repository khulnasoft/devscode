...
interface DevscodeAuthenticationSession extends AuthenticationSession {
  refreshToken: string;
  expiresInMs: number;
  loginNeeded: boolean;
}
...
export class WorkOsAuthProvider implements AuthenticationProvider, Disposable {
  ...
  private async storeSessions(value: DevscodeAuthenticationSession[]) {
  ...
  public async getSessions(
    scopes?: string[],
  ): Promise<DevscodeAuthenticationSession[]> {
    ...
    try {
      const value = JSON.parse(data) as DevscodeAuthenticationSession[];
      return value;
    } catch (e: any) {
  ...
  /**
   * Handle the redirect to VS Code (after sign in from Devscode)
   * @param scopes
   * @returns
   */
  private handleUri: (
