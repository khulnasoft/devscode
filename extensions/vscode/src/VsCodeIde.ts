...
class VsCodeIde implements IDE {
      ...
      if (!this.askedForAuth) {
        vscode.window
          .showInformationMessage(
            "Devscode will request read access to your GitHub email so that we can prevent abuse of the free trial. If you prefer not to sign in, you can use Devscode with your own API keys or local model.",
            "Sign in",
          ...
          .then(async (selection) => {
            if (selection === "Use API key / local model") {
              await vscode.commands.executeCommand(
                "devscode.devscodeGUIView.focus",
              );
              ...
              vscode.env.openExternal(
                vscode.Uri.parse(
                  "https://docs.devscode.dev/reference/Model%20Providers/freetrial",
                ),
              );
  ...
  getIdeInfo(): Promise<IdeInfo> {
    return Promise.resolve({
      ...
      extensionVersion:
        vscode.extensions.getExtension("devscode.devscode")?.packageJSON
          .version,
    });
  }
  ...
  async isTelemetryEnabled(): Promise<boolean> {
    ...
    const devscodeEnabled: boolean =
      (await vscode.workspace
        .getConfiguration(EXTENSION_NAME)
        .get("telemetryEnabled")) ?? true;
    return globalEnabled && devscodeEnabled;
  }
  ...
  async getClipboardContent() {
    return this.context.workspaceState.get("devscode.copyBuffer", {
      text: "",
