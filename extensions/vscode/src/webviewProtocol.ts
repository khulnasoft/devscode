        ...
        try {
          ...
          if (e.cause) {
            if (e.cause.name === "ConnectTimeoutError") {
              message = `Connection timed out. If you expect it to take a long time to connect, you can increase the timeout in config.json by setting "requestOptions": { "timeout": 10000 }. You can find the full config reference here: https://docs.devscode.dev/reference/config`;
            } else if (e.cause.code === "ECONNREFUSED") {
              message = `Connection was refused. This likely means that there is no server running at the specified URL. If you are running your own server you may need to set the "apiBase" parameter in config.json. For example, you can set up an OpenAI-compatible server like here: https://docs.devscode.dev/reference/Model%20Providers/openai#openai-compatible-servers--apis`;
            } else {
              message = `The request failed with "${e.cause.name}": ${e.cause.message}. If you're having trouble setting up Devscode, please see the troubleshooting guide for help.`;
            }
          }
          ...
          if (message.includes("https://proxy-server")) {
            ...
            if (message.includes("exceeded")) {
              message +=
                " To keep using Devscode, you can set up a local model or use your own API key.";
            }
              ...
              .then((selection) => {
                if (selection === "Show Logs") {
                  ...
                  vscode.env.openExternal(
                    vscode.Uri.parse(
                      "https://docs.devscode.dev/troubleshooting",
                    ),
                  );
                }
              });
