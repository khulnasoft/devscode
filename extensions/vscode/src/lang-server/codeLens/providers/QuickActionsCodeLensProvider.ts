import { DevscodeConfig, QuickActionConfig } from "core";
import { Telemetry } from "core/util/posthog";
import * as vscode from "vscode";

import { QuickEditShowParams } from "../../../quickEdit/QuickEditQuickPick";
import {
  DEVSCODE_WORKSPACE_KEY,
  getDevscodeWorkspaceConfig,
} from "../../../util/workspaceConfig";

const TUTORIAL_FILE_NAME = "devscode_tutorial.py";
...
export function getQuickActionsConfig(config: DevscodeConfig) {
  return config.experimental?.quickActions;
}

export function subscribeToVSCodeQuickActionsSettings(listener: Function) {
  vscode.workspace.onDidChangeConfiguration((e) => {
    const configKey = `${DEVSCODE_WORKSPACE_KEY}.${ENABLE_QUICK_ACTIONS_KEY}`;
...
export function toggleQuickActions() {
  ...
  getDevscodeWorkspaceConfig().update(ENABLE_QUICK_ACTIONS_KEY, curStatus);
}

export function quickActionsEnabledStatus() {
  return getDevscodeWorkspaceConfig().get<boolean>(ENABLE_QUICK_ACTIONS_KEY);
}
...
export class QuickActionsCodeLensProvider implements vscode.CodeLensProvider {
  ...
  getCustomCommands(
    ...
    return quickActionConfigs.map(({ title, prompt, sendToChat }) => {
      return sendToChat
        ? {
            title,
            command: "devscode.customQuickActionSendToChat",
            arguments: [prompt, range],
          }
        : {
            title,
            command: "devscode.customQuickActionStreamInlineEdit",
            arguments: [prompt, range],
          };
    });
  }
  ...
  getDefaultCommand(range: vscode.Range): vscode.Command[] {
    const quickEdit: vscode.Command = {
      command: "devscode.defaultQuickAction",
      title: "Devscode",
