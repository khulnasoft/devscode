...
export class VerticalDiffManager {
  ...
  // Listener for user doc changes is disabled during updates to the text document by devscode
  ...
  clearForFilepath(filepath: string | undefined, accept: boolean) {
    ...
    vscode.commands.executeCommand("setContext", "devscode.diffVisible", false);
  }
  ...
  async acceptRejectVerticalDiffBlock(
    ...
    // Disable listening to file changes while devscode makes changes
    this.disableDocumentChangeListener();
  ...
  async streamDiffLines(
    ...
    vscode.commands.executeCommand("setContext", "devscode.diffVisible", true);
  ...
  async streamEdit(
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by Devscode
      // Before adding this back, we need to distinguish between human and Devscode
