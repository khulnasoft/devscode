...
export class QuickEdit {
    ...
    quickPick.onDidAccept(async () => {
      ...
      switch (label) {
        case UserPromptLabels.AcceptAll:
          vscode.commands.executeCommand("devscode.acceptDiff", path);
          break;
        case UserPromptLabels.RejectAll:
          vscode.commands.executeCommand("devscode.rejectDiff", path);
          break;
        case QuickEditInitialItemLabels.Submit:
          if (quickPick.value) {
            await vscode.commands.executeCommand("devscode.rejectDiff", path);
  ...
  private handleQuickPickAccept = ({
    ...
    // If not an initial item, it's a file selection. Allow devscoded prompt editing.
