...
const statusBarItemText = (
  ...
  if (error) {
    return "$(alert) Devscode (FATAL ERROR)";
  }

  switch (status) {
    case undefined:
      if (loading) {
        return "$(loading~spin) Devscode";
      }
    case StatusBarStatus.Disabled:
      return "$(circle-slash) Devscode";
    case StatusBarStatus.Enabled:
      return "$(check) Devscode";
    case StatusBarStatus.Paused:
      return "$(debug-pause) Devscode";
  }
};
...
export function setupStatusBar(
  ...
  statusBarItem.command = "devscode.openTabAutocompleteConfigMenu";
  ...
  vscode.workspace.onDidChangeConfiguration((event) => {
    if (event.affectsConfiguration(DEVSCODE_WORKSPACE_KEY)) {
      const enabled = getDevscodeWorkspaceConfig().get<boolean>(
