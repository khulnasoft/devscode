...
import { DevscodeServerClient } from "core/devscodeServer/stubs/client";
...
import { DevscodeGUIWebviewViewProvider } from "./DevscodeGUIWebviewViewProvider";
...
function getFullScreenTab() {
  ...
  return tabs.find((tab) =>
    (tab.input as any)?.viewType?.endsWith("devscode.devscodeGUIView"),
  );
}
...
function focusGUI() {
  ...
  if (fullScreenTab) {
    ...
    // focus sidebar
    vscode.commands.executeCommand("devscode.devscodeGUIView.focus");
...
async function processDiff(
  ...
  sidebar: DevscodeGUIWebviewViewProvider,
...
const getCommandsMap: (
  ...
  diffManager: DiffManager,
  verticalDiffManager: VerticalDiffManager,
  devscodeServerClientPromise: Promise<DevscodeServerClient>,
  battery: Battery,
  quickEdit: QuickEdit,
  core: Core,
  ...
  return {
    ...
    "devscode.defaultQuickAction": async (args: QuickEditShowParams) => {
      captureCommandTelemetry("defaultQuickAction");
      vscode.commands.executeCommand("devscode.focusEdit", args);
    },
    "devscode.customQuickActionSendToChat": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "devscode.customQuickActionStreamInlineEdit": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "devscode.codebaseForceReIndex": async () => {
      core.invoke("index/forceReIndex", undefined);
    },
    "devscode.rebuildCodebaseIndex": async () => {
      core.invoke("index/forceReIndex", { shouldClearIndexes: true });
    },
    "devscode.docsIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: false });
    },
    "devscode.docsReIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: true });
    },
    "devscode.focusDevscodeInput": async () => {
      ...
      const isDevscodeInputFocused = await sidebar.webviewProtocol.request(
        "isDevscodeInputFocused",
        undefined,
      );

      if (isDevscodeInputFocused) {
        if (historyLength === 0) {
          hideGUI();
        } else {
        ...
        focusGUI();
        sidebar.webviewProtocol?.request("focusDevscodeInput", undefined);
        void addHighlightedCodeToContext(sidebar.webviewProtocol);
      }
    },
    "devscode.focusDevscodeInputWithoutClear": async () => {
      // This is a temporary fix—sidebar.webviewProtocol.request is blocking
      // when the GUI hasn't yet been setup and we should instead be
      // immediately throwing an error, or returning a Result object
      if (!sidebar.isReady) {
    ...
    "devscode.focusEdit": async (args?: QuickEditShowParams) => {
    ...
    "devscode.focusEditWithoutClear": async () => {
    ...
    //   quickEdit.show(args);
    // },
    "devscode.writeCommentsForCode": async () => {
    ...
    "devscode.writeDocstringForCode": async () => {
    ...
    "devscode.optimizeCode": async () => {
      captureCommandTelemetry("optimizeCode");
      streamInlineEdit("optimize", "Optimize this code");
    },
    "devscode.fixGrammar": async () => {
    ...
    "devscode.viewLogs": async () => {
      captureCommandTelemetry("viewLogs");

      // Open ~/.devscode/devscode.log
      const logFile = path.join(os.homedir(), ".devscode", "devscode.log");
    ...
    "devscode.sendMainUserInput": (text: string) => {
      sidebar.webviewProtocol?.request("userInput", {
        input: text,
      });
    },
    "devscode.selectRange": (startLine: number, endLine: number) => {
    ...
    "devscode.sendToTerminal": (text: string) => {
    ...
    "devscode.viewHistory": () => {
      sidebar.webviewProtocol?.request("viewHistory", undefined);
    },
    "devscode.focusDevscodeSessionId": async (
      ...
      void sidebar.webviewProtocol?.request("focusDevscodeSessionId", {
        sessionId,
      });
    },
    "devscode.applyCodeFromChat": () => {
      void sidebar.webviewProtocol.request("applyCodeFromChat", undefined);
    },
    "devscode.toggleFullScreen": () => {
      focusGUI();
      ...
      panel.onDidDispose(
        () => {
          ...
          vscode.commands.executeCommand("devscode.focusDevscodeInput");
        },
        null,
    ...
    "devscode.selectFilesAsContext": async (
      firstUri: vscode.Uri,
      uris: vscode.Uri[],
    ) => {
      if (uris === undefined) {
    ...
    "devscode.logAutocompleteOutcome": (
      completionId: string,
      completionProvider: CompletionProvider,
    ) => {
      completionProvider.accept(completionId);
    },
    "devscode.toggleTabAutocompleteEnabled": () => {
    ...
    "devscode.openTabAutocompleteConfigMenu": async () => {
      ...
      quickPick.onDidAccept(() => {
        ...
        if (targetStatus !== undefined) {
          ...
          vscode.commands.executeCommand("devscode.giveAutocompleteFeedback");
          ...
          focusGUI();
          vscode.commands.executeCommand("devscode.navigateTo", "/more", true);
        }
        quickPick.dispose();
      });
      quickPick.show();
    },
    "devscode.giveAutocompleteFeedback": async () => {
      const feedback = await vscode.window.showInputBox({
        ignoreFocusOut: true,
        prompt:
          "Please share what went wrong with the last completion. The details of the completion as well as this message will be sent to the Devscode team in order to improve.",
      });
      if (feedback) {
        const client = await devscodeServerClientPromise;
    ...
    "devscode.openMorePage": () => {
      vscode.commands.executeCommand("devscode.navigateTo", "/more", true);
    },
    "devscode.navigateTo": (path: string, toggle: boolean) => {
      sidebar.webviewProtocol?.request("navigateTo", { path, toggle });
      focusGUI();
    },
    "devscode.signInToControlPlane": () => {
      sidebar.webviewProtocol?.request("signInToControlPlane", undefined);
    },
    "devscode.openAccountDialog": () => {
...
const registerCopyBufferSpy = (context: vscode.ExtensionContext) => {
  ...
  async function doCopy(typeDisposable: any) {
    ...
    await context.workspaceState.update("devscode.copyBuffer", {
      text: clipboardText,
      copiedAt: new Date().toISOString(),
    });

    // re-register to devscode intercepting copy commands
