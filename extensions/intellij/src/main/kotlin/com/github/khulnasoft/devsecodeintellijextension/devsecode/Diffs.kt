...
import com.github.khulnasoft.devscodeintellijextension.utils.getAltKeyLabel
...
fun getDiffDirectory(): File {
    ...
    val diffDirPath = Paths.get(homeDirectory).resolve(".devscode").resolve(".diffs").toString()
...
class DiffManager(private val project: Project) : DumbAware {
    ...
    fun acceptDiff(file2: String?) {
        ...
        devscodePluginService.ideProtocolClient?.sendAcceptRejectDiff(true, diffInfo.stepIndex)
    ...
    fun rejectDiff(file2: String?) {
        ...
        devscodePluginService.ideProtocolClient?.deleteAtIndex(diffInfo.stepIndex)
        devscodePluginService.ideProtocolClient?.sendAcceptRejectDiff(false, diffInfo.stepIndex)
    ...
    private fun openDiffWindow(
        ...
        val diffRequest = SimpleDiffRequest("Devscode Diff", content1, content2, "Old", "New")
