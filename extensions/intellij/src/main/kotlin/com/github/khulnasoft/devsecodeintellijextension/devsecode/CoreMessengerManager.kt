...
class CoreMessengerManager(
  ...
  init {
    coroutineScope.launch {
      ...
      val devscodeCorePath =
          Paths.get(targetPath, "devscode-binary" + (if (os == "win32") ".exe" else "")).toString()

      setupCoreMessenger(devscodeCorePath)
    }
  }

  private fun setupCoreMessenger(devscodeCorePath: String): Unit {
    coreMessenger = CoreMessenger(project, devscodeCorePath, ideProtocolClient, coroutineScope)
