...
import com.github.khulnasoft.devscodeintellijextension.services.TelemetryService
...
class CoreMessenger(
  ...
  private fun write(message: String) {
    try {
      ...
      println("Error writing to Devscode core: $e")
    }
  }
  ...
  private fun handleMessage(json: String) {
    ...
    if (MessageTypes.PASS_THROUGH_TO_WEBVIEW.contains(messageType)) {
      ...
      if (messageType == "getDefaultModelTitle") {
        ...
        val defaultModelTitle = devscodeSettingsService.devscodeState.lastSelectedInlineEditModel
        val message =
            gson.toJson(
                mapOf(
      ...
      devscodePluginService.sendToWebview(messageType, responseMap["data"], messageType)
    }
  ...
  init {
    if (useTcp) {
      ...
      coroutineScope.launch(Dispatchers.IO) { setPermissions(devscodeCorePath) }

      // Start the subprocess
      val processBuilder =
          ProcessBuilder(devscodeCorePath).directory(File(devscodeCorePath).parentFile)
