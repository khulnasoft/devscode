...
class AutocompleteService(private val project: Project) {
    ...
    fun triggerCompletion(editor: Editor) {
        ...
        if (!settings.devscodeState.enableTabAutocomplete) {
            return
        }
        ...
        project.service<DevscodePluginService>().coreMessenger?.request(
            ...
            WriteAction.run<Throwable> {
                ...
                if (completion.lines().size > 1) {
                    editor.inlayModel.addBlockElement(
                        offset,
                        properties,
                        DevscodeMultilineCustomElementRenderer(editor, completion)
                    )
                } else {
                    editor.inlayModel.addInlineElement(
                        offset,
                        properties,
                        DevscodeCustomElementRenderer(editor, completion)
                    )
                }
...
//                }
//                val key = TextAttributesKey.createTextAttributesKey("DEVSCODE_AUTOCOMPLETE")
...
//                editor.markupModel.addLineHighlighter(key, editor.caretModel.logicalPosition.line, HighlighterLayer.LAST)
    ...
    private fun cancelCompletion(completion: PendingCompletion) {
        ...
        widget?.setLoading(false)
        project.service<DevscodePluginService>().coreMessenger?.request("autocomplete/cancel", null, null, ({}))
    }
    ...
    fun clearCompletions(editor: Editor) {
        ...
        editor.inlayModel.getInlineElementsInRange(0, editor.document.textLength).forEach {
            if (it.renderer is DevscodeCustomElementRenderer) {
                it.dispose()
            }
        }
        editor.inlayModel.getBlockElementsInRange(0, editor.document.textLength).forEach {
            if (it.renderer is DevscodeMultilineCustomElementRenderer) {
                it.dispose()
            }
        }
    }
