...
@Service
class DevscodeAuthService {
    private val coroutineScope = CoroutineScope(Dispatchers.IO)

    companion object {
        fun getInstance(): DevscodeAuthService = service<DevscodeAuthService>()
        private const val CREDENTIALS_USER = "DevscodeAuthUser"
        private const val ACCESS_TOKEN_KEY = "DevscodeAccessToken"
        private const val REFRESH_TOKEN_KEY = "DevscodeRefreshToken"
        private const val ACCOUNT_ID_KEY = "DevscodeAccountId"
        private const val ACCOUNT_LABEL_KEY = "DevscodeAccountLabel"
...
//        private const val CONTROL_PLANE_URL = "http://localhost:3001"
    ...
    init {
        val settings = service<DevscodeExtensionSettings>()
        if (settings.devscodeState.enableDevscodeTeamsBeta) {
    ...
    fun startAuthFlow(project: Project) {
        ...
        ApplicationManager.getApplication().invokeLater {
            val dialog = DevscodeAuthDialog() { token ->
                // Store the token
    ...
    private fun openSignInPage(project: Project) {
        val coreMessenger = project.service<DevscodePluginService>().coreMessenger
