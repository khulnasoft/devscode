...
class AutocompleteSpinnerWidget(project: Project) : EditorBasedWidget(project), StatusBarWidget.IconPresentation,
    ...
    init {
        Disposer.register(DevscodePluginDisposable.getInstance(project), this)
        updateIcon()
    }
    ...
    override fun getTooltipText(): String? {
        val enabled = service<DevscodeExtensionSettings>().state.enableTabAutocomplete
        return if (enabled) "Devscode autocomplete enabled" else "Devscode autocomplete disabled"
    }
    ...
    override fun getIcon(): Icon = if (isLoading) animatedIcon else
        IconLoader.getIcon("/icons/devscode.svg", javaClass)
