    ...
    for entry in fs::read_dir(workspace_dir)? {
        ...
        if path.is_dir() {
            ...
            match path.file_name().and_then(|name| name.to_str()) {
                Some(file_name) if file_name.ends_with(".gitignore") || file_name.ends_with(".devscodeignore") => {
...
mod tests {
    ...
    fn test_local_find_gitignores() -> io::Result<()> {
        ...
        // Create some directories and .gitignore/.devscodeignore files
        let dir_structure = [
            ("dir1", Some(".gitignore"), "node_modules/"),
            ("dir2", Some(".devscodeignore"), "target/"),
            ("dir3", None, ""),
        ];
    ...
    #[test]
    fn test_in_devscode_repo() -> io::Result<()> {
