...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
        ...
        if (!getLocalStorage("shownProfilesIntroduction")) {
          ...
          dispatch(
            setDialogMessage(
              <ConfirmationDialog
                text={
                  "Welcome to Devscode for teams! Using account icon in the top right, you can switch between your local profile (defined by config.json) and team profiles (defined in the Devscode for teams web app). Each profile defines a set of models, slash commands, context providers, and other settings to customize Devscode."
                }
    ...
    dispatch(
      setDialogMessage(
        <ConfirmationDialog
          ...
          text={"Are you sure you want to log out of Devscode?"}
          onConfirm={() => {
  ...
  useEffect(() => {
    ideMessenger.ide.getIdeSettings().then(({ enableControlServerBeta }) => {
      ...
      if (shouldShowPopup) {
        ideMessenger.ide.showToast("info", "Devscode for Teams enabled");
      }
    });
  }, []);
